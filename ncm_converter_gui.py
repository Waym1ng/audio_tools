...
class NCMConverterApp(ctk.CTk):
    def __init__(self):
        ...
        ctk.CTkButton(self, text="选择文件/目录", command=self.select_ncm).pack(pady=5)

        ctk.CTkLabel(self, text="💾 输出目录：").pack(pady=(15, 5))
        ctk.CTkEntry(self, textvariable=self.output_dir, width=480).pack()
        ctk.CTkButton(self, text="选择输出目录", command=self.select_output).pack(pady=5)

        ctk.CTkButton(self, text="🚀 开始转换", command=self.start_conversion).pack(pady=20)

        self.status_label = ctk.CTkLabel(self, textvariable=self.status_text, wraplength=520, justify="left", text_color="green")
        self.status_label.pack(pady=10)

    def select_ncm(self):
        path = filedialog.askopenfilename(title="选择单个 .ncm 文件", filetypes=[("NCM 文件", "*.ncm")])
        if not path:
            path = filedialog.askdirectory(title="或选择包含 NCM 的目录")
        if path:
    ...
    def start_conversion(self):
        ...
        if os.path.isfile(src) and src.endswith(".ncm"):
            ...
            ncm_files = [
                str(f) for f in Path(src).rglob("*.ncm")
            ]
        else:
        ...
        for i, file in enumerate(ncm_files, 1):
            self.status_text.set(f"正在转换 ({i}/{len(ncm_files)}): {os.path.basename(file)}")
            self.update()
        ...
        self.status_text.set(f"🎉 转换完成！成功：{success_count} 个，失败：{fail_count} 个。")
